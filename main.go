...
func Incremental(logger *zap.Logger) error {
	...
	if isTerminal {
		...
		// small amount of time for it to bind to the localhost dbPort
	...
	// WARNING: This will print out the password, but might be useful for local debugging
	// fmt.Println(connStr)
	...
	for _, tableName := range tableList {
		err = processSingleTable(ctx, logger, client, conn, gcsClient, bqProject, tableName)
		if err != nil {
